...
class RecursiveViTConfig(pydantic.BaseModel):
    image_size: int
    patch_size: int
    num_classes: int
    dim: int
...
class RecursiveViTInner(nn.Module):
    ...
    def init_state(self, batch_size: int) -> RecursiveViTState:
        ...
        if self.config.state_init == 'noise':
            trunc_normal_init_(initial_state_tensor)
        elif self.config.state_init == 'zero':
            initial_state_tensor.zero_()
        else:
            raise ValueError(f"Invalid state initialization method: {self.config.state_init}")

        return RecursiveViTState(step=0, state=initial_state_tensor)

    def forward(self, img: torch.Tensor, prev_state: RecursiveViTState) -> tuple[torch.Tensor, RecursiveViTState]:
        """
        Args:
            img: (B, C, H, W) images
            prev_state: RecursiveViTState with state of shape (B, state_dim, dim)
        Returns:
        ...
        # Fix 2: Extract state BEFORE pooling
        if self.config.class_token_in_state:
...
class RecursiveViT(nn.Module):
    ...
    def forward(self, img: torch.Tensor) -> torch.Tensor:
        state = self.inner.init_state(img.size(0))
