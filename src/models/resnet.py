...
class ResNetConfig(pydantic.BaseModel):
    num_classes: int
    size: int

class ResNet(_ResNet):
    def __init__(self, config: ResNetConfig):
        # Determine block type and layers based on size
        if config.size == 18:
            block, num_blocks = BasicBlock, [2,2,2,2]
        elif config.size == 34:
            block, num_blocks = BasicBlock, [3,4,6,3]
        elif config.size == 50:
            block, num_blocks = Bottleneck, [3,4,6,3]
        elif config.size == 101:
            block, num_blocks = Bottleneck, [3,4,23,3]
        elif config.size == 152:
